...
public class MyDialog<R> implements EventTarget {
     ...
     * a new one.
     */
    private ObjectProperty<Pane> dialogPane = new SimpleObjectProperty<Pane>(this, "dialogPane", new Pane()) {
        final InvalidationListener expandedListener = o -> {
        	Pane dialogPane = getDialogPane();
            if (dialogPane == null) return;

            final Node content = dialogPane.getChildren().get(0); //dialogPane.getExpandableContent();
        ...
        WeakReference<Pane> dialogPaneRef = new WeakReference<>(null);

        @Override
        protected void invalidated() {
            Pane oldDialogPane = dialogPaneRef.get();
            if (oldDialogPane != null) {
            }

            final Pane newDialogPane = getDialogPane();


            // push the new dialog down into the implementation for rendering
            dialog.setDialogPane(newDialogPane);

            dialogPaneRef = new WeakReference<Pane>(newDialogPane);
        }
    };

    public final ObjectProperty<Pane> dialogPaneProperty() {
        return dialogPane;
    }

    public final Pane getDialogPane() {
        return dialogPane.get();
    }

    public final void setDialogPane(Pane value) {
